...
EXPECTED_MANIFEST_ID = "OAB_FIXTURE_INGRESS_MANIFEST_v0.2"
EXPECTED_PRIOR_MANIFEST_ID = "OAB_FIXTURE_INGRESS_MANIFEST_v0.1"
EXPECTED_PRIOR_CONTRACT_ID = "OAB_FIXTURE_LOADER_CONTRACT_v0.1"
EXPECTED_BATTERY_ID = "OMEGA_ORIENTATION_ADVERSARIAL_BATTERY_01"
EXPECTED_FIXTURE_SCHEMA_VERSION = "OAB_FIXTURE_SHAPE_v0.2"
EXPECTED_LOADER_CONTRACT_VERSION = "OAB_FIXTURE_LOADER_v0.2"
...
def _validate_schema(fixture: Mapping[str, Any]) -> None:
    ...
    if errors:
        ...
        raise FixtureIngressError(f"SCHEMA_REJECT:{str(first.validator).upper()}:{path}:{first.message}")


def _validate_manifest(manifest: Mapping[str, Any]) -> None:
    expected_keys = {
        "manifest_id", "prior_manifest_id", "prior_contract_id", "battery_id",
        "fixture_schema_version", "loader_contract_version", "semantic_bindings",
        "fixture_root", "fixture_ids", "fixture_sha256", "authority", "boundary_law",
    }
    ...
    if manifest["prior_manifest_id"] != EXPECTED_PRIOR_MANIFEST_ID:
        raise FixtureIngressError("manifest:PRIOR_MANIFEST_ID_MISMATCH")
    if manifest["prior_contract_id"] != EXPECTED_PRIOR_CONTRACT_ID:
        raise FixtureIngressError("manifest:PRIOR_CONTRACT_ID_MISMATCH")
...
def load_fixture_bytes(raw_fixture: bytes, *, source_path: str, manifest: Mapping[str, Any]) -> LoadedFixture:
    """Parse and validate one v0.2 fixture without causal interpretation or execution."""
    ...
    if digest != manifest["fixture_sha256"][fixture_id]:
    ...
    return LoadedFixture(
        ...
        prior_manifest_id=manifest["prior_manifest_id"],
        prior_contract_id=manifest["prior_contract_id"],
...
__all__ = ["FixtureIngressError", "LoadedFixture", "LoaderDisposition", "load_fixture_bytes", "load_fixture_file"]
