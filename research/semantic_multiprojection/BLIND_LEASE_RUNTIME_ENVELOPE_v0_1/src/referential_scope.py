...
class ReferentialScopeResult:
    ...
    @property
    def closed(self) -> bool: return self.source_complete and self.preservation_complete and self.chamber_set_complete
...
def verify_referential_scope(source: dict, preservation: dict, chamber_ids: set[str]) -> ReferentialScopeResult:
    ...
    preservation_complete = all(REQUIRED_SOURCE_COORDINATES <= set(contract) for name, contract in preservation.items() if name.startswith("Omega_")) and bool(preservation)
    normalized = {cid.split("_", 1)[0] for cid in chamber_ids}
    return ReferentialScopeResult(source_complete, preservation_complete, normalized == REQUIRED_CHAMBERS)
