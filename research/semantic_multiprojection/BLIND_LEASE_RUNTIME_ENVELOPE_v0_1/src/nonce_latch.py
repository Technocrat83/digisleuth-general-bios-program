...
class DispatchLatch:
    ...
    def consume(self, *, now: float, nonce: str) -> "DispatchLatch":
        ...
        if state != "ISSUED":
            raise DispatchLatchError(f"dispatch authorization not consumable: {state}")
        if not secrets.compare_digest(self.nonce, nonce):
    ...
    def revoke(self) -> "DispatchLatch":
        if self.state == "CONSUMED":
            raise DispatchLatchError("consumed dispatch cannot be retroactively revoked")
