...
class MechanicalPreflight:
    ...
    @property
    def passed(self)->bool: return self.battery_witness.manifest_verified and self.battery_witness.unchanged and self.terminal_states_closed

def run_mechanical_preflight(battery_root:Path)->MechanicalPreflight:
    witness=read_only_integrity_inspection(battery_root)
    return MechanicalPreflight(witness,TERMINAL_STATES=={"COMPLETED","ABSTAINED","INTEGRITY_HALT","BLINDNESS_BREACH","EXECUTION_FAULT"})
